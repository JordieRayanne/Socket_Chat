...
public class Social extends JFrame {
	...
	//the rigth-side panel with the file listener
		...
		fileSend.setBounds(5,30,150,50);
		fileSend.addActionListener((ActionEvent e)->{
				new Fichier(soc2);
			}
		);
		menu.add(fileSend);
		///to show the online filemanager
		fileManager=new JButton("File manager");
		fileManager.setBounds(5,85,150,50);
		menu.add(fileManager);
		///to play online stored media
		media=new JButton("Play media");
		media.setBounds(5,140,150,50);
		menu.add(media);
		///to upload media
		upload=new JButton("Upload media");
		upload.setBounds(5,195,150,50);
		menu.add(upload);
